...
Directory contains all processed data reported in the manuscript, as well as the Jupyter Notebooks used to generate all plots for all figures in the manuscript. The directory tree below lists all files and a description of what each is used for. To analyze the processed data in ```data/```, run the Jupyter notebooks. 

```
README.md       # This file

data/   # Contains processed data from which all figers were made

    molecular_dynamics_simulations/ # Initial drug structures (.pdb) 
                                    # MD simiulation output (.xtc)
    ...
    rna-seq/    # RMATs analysis of RNA-seq data, and inferred allelic manifolds
...
fig3/
    ...
    fig3_S11_S12_inference.ipynb    # Do joint model inference
                                    # Must be run before fig3C-H_S12.ipynb
...
fig5/
    ...
    fig5E-L_6A-K_inference.ipynb  # Dose-response curve inference, Figs. 5, 6;
                                  # Must be run before fig5D-L.ipynb, fig6.ipynb
